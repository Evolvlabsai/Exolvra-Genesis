...
export const HELP_TOPICS: readonly HelpTopic[] = [
  {
    ...
    body: [
      ...
      'runtime, including run, interview, chart and builder/critic instructions. When',
      ...
      '',
      'GITHUB_TOKEN: the credential for `work`, `queue` and GitHub `chart` requests.',
      'When it is unset, `gh auth token` is asked instead, and a',
      'run with neither is a configuration error. It is never written to a run record,',
      'a comment, a pull request body or a progress page, and anything shaped like a',
      'token is removed from what those surfaces show.',
      '',
      'GITHUB_API_URL: the GitHub REST API to talk to, for GitHub Enterprise Server or',
      'a proxy. It defaults to https://api.github.com, and every request goes to the',
      'host it names and to no other — a redirect somewhere else is refused rather',
      'than followed. GitHub Actions sets it on every runner. Only https is accepted,',
      'except on this machine, where http is allowed because loopback is not a network.',
      '',
      'EXOLVRA_GENESIS_REPOS: the repositories one invocation may look at, as a',
      'comma-separated or space-separated list of owner/name. A --repo flag overrides',
      'it outright rather than adding to it. Issue commands require a repository;',
      'they never search every repository the token can see. For `chart`, a configured',
      'repository selects GitHub storage by default. Multiple configured repositories',
      'require --repo; --tracker local always selects local map files.',
      '',
      'EXOLVRA_GENESIS_RUNNER_LOGIN: the GitHub account `work` posts as and GitHub',
      '`chart` claims tickets as, for a token',
      'GitHub will not name. A user token answers for itself and this is not needed;',
      'an installation or Actions token is refused GET /user, and the login its',
      'comments carry — <app-slug>[bot] — has to be given here or with --runner-login.',
      'Every label, comment, branch and pull request is decided against a status',
      'comment the runner has to recognise as its own, so a run that can name neither',
      'itself nor an operator-supplied account exits before it reads an issue. It is',
      'a name, not a credential. Read-only surfaces do not need it.',
      '',
      'EXOLVRA_GENESIS_AUTO_RESUMES: how many times `run` and `resume` re-drive a',
      'session that ended abnormally — a stream fault, or a turn that ended while the',
      'run still said `running` — before handing the resume line back to you. A',
      'deliberate ending (a lead that blocked, a tripped budget guard, an interrupt)',
      'is never re-driven. Default 2; 0 turns recovery off; values above 5 are read',
      'as 5, because a fault that survives five fresh sessions is not going away on',
      'its own. Attempt N waits N x EXOLVRA_GENESIS_AUTO_RESUME_DELAY_MS first',
      '(default 15000), because the commonest fault is the API saying "Overloaded"',
      'and an instant retry asks the same servers the same question in the same',
      'moment.',
      '',
      'EXOLVRA_GENESIS_STALL_LEAD_SECONDS: lead inactivity threshold, default 180.',
      'EXOLVRA_GENESIS_STALL_BUILDER_SECONDS: builder threshold, default 300.',
      'EXOLVRA_GENESIS_STALL_CRITIC_SECONDS: critic threshold, default 180.',
      'EXOLVRA_GENESIS_STALL_VERIFICATION_SECONDS: verification threshold, default 60.',
      'Set positive seconds; other values use the default. These thresholds flag',
      'inactivity in live status and progress pages; the watchdog does not kill runs.',
      '`status --stall-seconds` overrides all four for that view. Its',
      '--verification-stall-seconds flag overrides only verification when no',
      '--stall-seconds value is supplied.',
      '',
      'EXOLVRA_GENESIS_COORDINATOR: optional shared-directory path for distributed',
      'builder/critic rounds. `run` and `resume` can override it with --coordinator;',
      '`work` inherits it. Worker daemons need access to the same authenticated',
      'shared filesystem. When unset, rounds use the local SDK.',
      '',
  ...
  },
  {
    ...
    body: [
      ...
      '  exit code will be 2',
      '',
      'Execution permission preflight refusals and an unavailable requested model',
      '(including an inherited Claude default) also exit 2: change the invocation.',
      '',
