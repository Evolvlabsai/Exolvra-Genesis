...
/gauntlet:run <goal or path to spec file>
```

Prefix the arguments with `auto` to skip the bar-approval pause. Headless run:

```
claude -p "/gauntlet:run auto specs/foo.md" --permission-mode acceptEdits --max-turns 80
```
...
- `commands/run.md` — the lead agent (`/gauntlet:run`). Owns orchestration only: captures the bar into `.gauntlet/bar/` (immutable per run), decomposes the goal (or a spec file) into Task Specs, fans out builders, re-runs their verification commands itself, dispatches fresh critics with shuffled A/B labels, and enforces the win condition (two consecutive blind wins). The lead never writes implementation code.
- `agents/builder.md` (`gauntlet-builder`) — implements exactly one Task Spec end to end. Touches only its owned files; no stubs, no faked evidence. Reports only FILES CHANGED / COMMANDS RUN / VERIFICATION (verbatim output).
- `agents/critic.md` (`gauntlet-critic`) — blind judge. Sees only the bar and the real output, never builder code or prior rounds. Reports only VERDICT (WIN/LOSS) / GAP (single biggest, one sentence) / EVIDENCE. A tie is a LOSS.
...
Run state lives under `.gauntlet/` in the *target* project (gitignored here and recommended to users): `bar/` captures, `state.json` (`{"status": "running" | "complete" | "stopped"}`), `progress.html` (self-contained live dashboard), `runs/` (per-round snapshots).
...
- Spec files passed to `/gauntlet:run` are read-only for the run; full requirement coverage is a hard gate for the assembled result.
